...
def run(seed, mut, rpl, M, N, a, mu, w):

    cdef int y, m, rec, s, A, yTot, yRec

    #create bit and log file
    filepath = '/home/edith/Documents/GA/results/rw/' 
    filename = "rw-a" + str(int(a*100)) + "M" + str(M) + '-N' + str(N) + '-w' + str(int(w*100)) + '-'
    if mut:
        mtype = "pc"
    else:
        mtype = "pl"
    ...
    while (y < yTot ): #condition until which the simulation runs
        ...
        Offspring = Selection.roulettewheel(Population, M, N, w, mut, mu)
