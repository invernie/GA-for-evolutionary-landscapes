...
cdef int seed, mut, rpl, M, N, rf
cdef float O, a, mu
random.seed(seed)
np.random.seed(seed)


def run(seed, mut, rpl, M, N, a, mu, rf, O):
    cdef int thr, y, m, rec, s, A, yTot, yRec

    #create bit and log file
    filepath = '/home/edith/Documents/GA/results/rank/' 
    if rf == 0:
        funtype = "lin"
    elif rf == 1:
        funtype = "exp"

    filename = "rank-a" + str(int(a*100)) + "M" + str(M) + '-N' + str(N) + '-' + funtype + str(int(O*100)) + '-'
    if mut:
        mtype = "pc"
    else:
        mtype = "pl"
    ...
    while (y < yTot ): #condition until which the simulation runs
        ...
        Offspring = Selection.ranking(Population, M, N, rf, O, mut, mu)
