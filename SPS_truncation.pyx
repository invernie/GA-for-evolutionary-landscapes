...
def run(seed, mut, rpl, M, N, a, h, mu):
    cdef int thr, y, m, rec, s, A, yTot, yRec

    #create bit and log file
    filepath = '/home/edith/Documents/GA/results/tr/' 
    filename = "tr-a" + str(int(a*100)) + "M" + str(M) + '-N' + str(N) + '-tr' + str(int(h*100)) + '-'
    if mut:
        mtype = "pc"
    else:
        mtype = "pl"
    ...
    while (y < yTot ): #condition until which the simulation runs
        ...
        thr = __builtins__.int(round(h*M))
        Offspring = Selection.truncation(Population, thr, mut, N, mu)
